...
contract BundledYieldVaultV2_PRODUCTION is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        if (_tydroPool == address(0) || _acrossSpokePool == address(0) || _l2Encoder == address(0)) revert InvalidAddress();
        if (_l1Recipient == address(0)) revert InvalidAddress();
        if (_veloRouter == address(0) || _slipstreamPositionNFT == address(0)) revert InvalidAddress();
        ...
        SlipstreamHelper slipHelper = new SlipstreamHelper(_slipstreamPositionNFT);
        ...
        maxOperationsPerHour = 69; ///?... prob remove this.. too extreme lol
        emergencyMode = false;
        breakerActive = false;

        _initializeOwner(msg.sender);

        emit VaultInitialized(_tydroPool, _l2Encoder, _acrossSpokePool, _l1Recipient);
        emit VelodromeRouterSet(_veloRouter);
    }

    /// @notice Auto-deposit available bridged funds (anyone can call - keeper-friendly)
    ...
    /// @dev Otherwise, deposits to Tydro (backward compatible)
        ...
        rateLimitCheck(token)
    {
        ...
        if (currentBalance <= depositedAmount) {
            emit AutoDepositSkipped(token, currentBalance, depositedAmount, msg.sender);
            return;
        }
        ...
        ///Use smart allocation if enabled and allocator is set
        if (useSmartAllocation && address(yieldAllocator) != address(0)) {
            SafeTransferLib.safeApprove(token, address(yieldAllocator), newAmount);
            yieldAllocator.allocateFunds(token, newAmount, 0);///0 = auto-select best strategy
            emit AutoDeposited(token, newAmount, msg.sender, true);
        } else {
            ///Default: deposit to Tydro (backward compatible)
            _depositToTydro(token, newAmount);
            emit AutoDeposited(token, newAmount, msg.sender, false);
        }
    }
    
    /// @notice Auto-deposit available bridged funds to Tydro (backward compatible, no smart allocation)
        ...
        withdrawalLimitCheck(token, amount)
    {
        ...
        if (maxDeposits[token] > 0) {
            TokenStatus storage status = tokenStatus[token];
            if (status.depositedAmount + amount > maxDeposits[token]) revert DepositLimitExceeded();
        }
           ...
           ///refill if needed
            ...
            if (actualBridgeAmount > 0) {
               ///Recalculate minAmountOut for reduced amount
                ...
                uint256 currentAllowance = _erc20Allowance(token, ACROSS_SPOKE_POOL);
               ...
               ///Try depositV3 first... fallback to legacy if needed
               ...
               ///Try depositV3
                try this._tryDepositV3(
                   ...
                   ///Log err but continue legacy deposit
                    if (reason.length >= 4) {
                        bytes4 selector = bytes4(reason);
                        emit BridgeError(token, selector, actualBridgeAmount, minAmountOut);
                    }
                }
               ///Fallback to legacy depo
                if (!bridgeSuccess) {
                    try this._tryDepositLegacy(
                        token,
                        l1Token,
                        actualBridgeAmount,
                        minAmountOut,
    ...
    function harvestAndBridge(
        ...
        rateLimitCheck(token)
    {
        _harvestAndBridgeInternal(token, compoundPercent, customSlippageBps, minBridgeAmount);
    }
        ...
        nonReentrant
    {
        ...
        if (operationCooldown[token] > 0) {
            if (block.timestamp < lastOperationTime[token] + operationCooldown[token]) {
        ...
        if (principal == 0) revert InsufficientYield();
       ///Check minimum threshold (e.g. 0.1% of principal)
       ...
       ///Increment rate limits AFTER checks pass (before expensive ops)
        if (maxOperationsPerHour > 0) {
            uint32 currentHour = uint32(block.timestamp / 3600);
       ...
       ///Note: _harvestAndBridgeInternal will call _updateYield again
       ///as it ensures we have the laaaaaaaaaatest yield before harvesting
        ...
        rateLimitCheck(token)
    {
       ///auto-deposit any available bridged funds (call via this to access external function)
        ...
        this.harvestAndBridge(token, compoundPercent, customSlippageBps, minBridgeAmount);
    }
    ...
    function smartRebalance(address token) external onlyOwner whenNotPaused nonReentrant rateLimitCheck(token) {
        if (emergencyMode) revert EmsModeInit();
    ...
    function smartCompound(address token) external onlyOwner whenNotPaused nonReentrant rateLimitCheck(token) {
        if (emergencyMode) revert EmsModeInit();
        if (address(yieldAllocator) == address(0)) revert AllocatorNotSet();
    ...
    function _tryDepositV3(
       ...
       ///Get quote timestamp buffer and calculate valid quoteTimestamp
        uint32 quoteTimeBuffer = ISpokePool(ACROSS_SPOKE_POOL).depositQuoteTimeBuffer();
       ...
       ///Validate quoteTimestamp is within buffer
        if (quoteTimestamp > uint32(currentTime + quoteTimeBuffer) || 
            quoteTimestamp < uint32(currentTime - quoteTimeBuffer)) {
            revert BridgeFailed();
        }
        uint32 fillDeadlineBuffer = ISpokePool(ACROSS_SPOKE_POOL).fillDeadlineBuffer();
    ...
    function getVaultHealth(address token) external view returns (
        bool isHealthy,
        bool hasGas,
        bool hasYield,
        ...
        if (aToken == address(0)) {
            try IL2Pool(TYDRO_POOL).getReserveData(token) returns (
                ...
                uint256 timeSinceUpdate = block.timestamp > status.lastUpdate ? block.timestamp - status.lastUpdate : 0;
    ...
    function getTotalValueLocked(address[] calldata tokens) external view returns (uint256 total) {
        for (uint256 i = 0; i < tokens.length; i++) {
    ...
    function canPerformOp(address token) external view returns (bool canOperate, string memory reason) {
        ...
        if (tokenMapping[token] == address(0)) return (false, "Token not supported");
        if (address(this).balance < minGasBalance) return (false, "Insufficient gas");
        if (maxOperationsPerHour > 0) {
            uint32 currentHour = uint32(block.timestamp / 3600);
            if (operationsThisHour[currentHour] >= maxOperationsPerHour) {
    ...
    function estimateBridgeFee(address token, uint256 amount) external view returns (uint256 estimatedFee) {
    ...
    function momoCompound(address token, uint256 minYieldThreshold) external onlyOwner whenNotPaused nonReentrant rateLimitCheck(token) {
        if (emergencyMode) revert EmsModeInit();
        if (tokenMapping[token] == address(0)) revert TokenNotSupported();
        _updateYield(token);
        TokenStatus storage status = tokenStatus[token];
    ...
    function getCurrentAPY(address token) external view returns (uint256 apyBps) {
        try IL2Pool(TYDRO_POOL).getReserveData(token) returns (
            uint256, uint128 liquidityIndex, uint128 currentLiquidityRate, uint128, uint128, uint128, uint40, uint16,
           ...
           ///currentLiquidityRate is in ray (1e27), 
            uint256 annualRate = (uint256(currentLiquidityRate) * 365 days) / 1e9;///Convert ray to annual
    ...
    function findBestYieldToken(address[] calldata tokens) external view returns (address bestToken, uint256 bestAPY) {
        bestAPY = 0;
        bestToken = address(0);
        for (uint256 i = 0; i < tokens.length; i++) {
    ...
    function createVelodromeLP(
        ...
        uint256 liquidity = IVelodromeHelper(VELO_HELPER).createLP(tokenA, tokenB, amountA, amountB, stable, shouldStake);
       ///Optional: Send LP to owner's EOA 4 ui
        if (sendToOwner) {
            address pair = IVeloRouter(VELO_ROUTER).pairFor(tokenA, tokenB, stable);
            if (pair != address(0)) {
                IVelodromeHelper(VELO_HELPER).transferLP(pair, owner(), liquidity);
    ...
    /// @notice Zap into LP 
    /// @param sendToOwner If true, LP tokens go to owner's EOA just for now(visible in velo UI). If false, stay in vault (automated).
    function zapIntoLP(
        address tokenIn,
        address tokenOut,
        uint256 amountIn,
       ...
       ///If sending to owner, don't stake in gauge (owner can stake manuallyif they want)
        bool shouldStake = stakeInGauge && !sendToOwner;
        uint256 liquidity = IVelodromeHelper(VELO_HELPER).zapIntoLP(tokenIn, tokenOut, amountIn, stable, shouldStake, minLiquidity);
       ///Optional: Send LP to owner's EOA 4 ui
        if (sendToOwner) {
    ...
    function setSlipstreamGauge(address tokenA, address tokenB, uint24 fee, address gauge) external onlyOwner {
        ISlipstreamHelper(SLIPSTREAM_HELPER).setLeafGauge(tokenA, tokenB, fee, gauge);
    ...
    {
        if (emergencyMode) revert EmsModeInit();
        if (params.token0 == address(0) || params.token1 == address(0)) revert InvalidAddress();
        if (tokenMapping[params.token0] == address(0)) revert TokenNotSupported();
        if (tokenMapping[params.token1] == address(0)) revert TokenNotSupported();
        if (params.amount0Desired == 0 && params.amount1Desired == 0) revert InvalidAmount();
        if (params.amount0Desired > 0) {
            if (_erc20Balance(params.token0) < params.amount0Desired) revert InsufficientBalance();
            params.token0.safeTransfer(SLIPSTREAM_HELPER, params.amount0Desired);
        }
        if (params.amount1Desired > 0) {
            if (_erc20Balance(params.token1) < params.amount1Desired) revert InsufficientBalance();
            params.token1.safeTransfer(SLIPSTREAM_HELPER, params.amount1Desired);
        }

        ISlipstreamPositionNFT.MintParams memory mintParams = ISlipstreamPositionNFT.MintParams({
        ...
        uint256 tokenId = ISlipstreamHelper(SLIPSTREAM_HELPER).createPosition(mintParams, stakeInGauge);
        emit SlipstreamPositionCreated(tokenId, params.token0, params.token1, params.fee, stakeInGauge);
    }
    ...
    {
        if (emergencyMode) revert EmsModeInit();
        (address token0, address token1, , , , ) = ISlipstreamHelper(SLIPSTREAM_HELPER).getPosition(params.tokenId);
        ...
        if (operationCooldown[rateLimitToken] > 0) {
            if (block.timestamp < lastOperationTime[rateLimitToken] + operationCooldown[rateLimitToken]) {
                revert OperationCooldown();
            }
            lastOperationTime[rateLimitToken] = uint32(block.timestamp);
        }
        if (params.amount0Desired > 0) {
            if (_erc20Balance(token0) < params.amount0Desired) revert InsufficientBalance();
            token0.safeTransfer(SLIPSTREAM_HELPER, params.amount0Desired);
        }
        if (params.amount1Desired > 0) {
            if (_erc20Balance(token1) < params.amount1Desired) revert InsufficientBalance();
            token1.safeTransfer(SLIPSTREAM_HELPER, params.amount1Desired);
        }

        ISlipstreamPositionNFT.IncreaseLiquidityParams memory liqParams = ISlipstreamPositionNFT.IncreaseLiquidityParams({
            amount0Desired: params.amount0Desired,
            amount1Desired: params.amount1Desired,
            amount0Min: params.amount0Min,
            amount1Min: params.amount1Min,
        ...
        (uint256 liquidity, uint256 amount0, uint256 amount1) =
            ISlipstreamHelper(SLIPSTREAM_HELPER).increaseLiquidity(params.tokenId, liqParams);
        emit SlipstreamLiquidityIncreased(params.tokenId, liquidity, amount0, amount1);
    }
        ...
        nonReentrant
    {
        ...
        if (tokenMapping[token0] == address(0)) revert TokenNotSupported();
        if (tokenMapping[token1] == address(0)) revert TokenNotSupported();
        ISlipstreamPositionNFT.DecreaseLiquidityParams memory decParams = ISlipstreamPositionNFT.DecreaseLiquidityParams({
        ...
        (uint256 amount0, uint256 amount1) =
            ISlipstreamHelper(SLIPSTREAM_HELPER).decreaseLiquidity(params.tokenId, decParams);
        emit SlipstreamLiquidityDecreased(params.tokenId, params.liquidity, amount0, amount1);
    }

    /// @notice Collect Slipstream fees to vault
    function collectSlipstreamFees(uint256 tokenId)
        external
        onlyOwner
        whenNotPaused
        nonReentrant
    {
        if (emergencyMode) revert EmsModeInit();
        (address token0, address token1, , , , ) = ISlipstreamHelper(SLIPSTREAM_HELPER).getPosition(tokenId);
        if (token0 == address(0) || token1 == address(0)) revert InvalidAddress();
        if (tokenMapping[token0] == address(0)) revert TokenNotSupported();
        if (tokenMapping[token1] == address(0)) revert TokenNotSupported();
        (uint256 amount0, uint256 amount1) =
            ISlipstreamHelper(SLIPSTREAM_HELPER).collectFees(tokenId, address(this));
        ...
        returns (uint256 rewards)
    {
        ...
        rewards = ISlipstreamHelper(SLIPSTREAM_HELPER).harvestRewards(positionHash);
        ...
        nonReentrant
    {
        ...
        bytes32 positionHash = _slipstreamPositionHash(token0, token1, fee);
        ISlipstreamHelper(SLIPSTREAM_HELPER).stakePosition(tokenId, positionHash);
        emit SlipstreamPositionStaked(positionHash, tokenId);
    }

    /// @notice Unstake Slipstream position NFT from gauge
    function unstakeSlipstreamPosition(uint256 tokenId, address token0, address token1, uint24 fee)
        external
        onlyOwner
        whenNotPaused
        nonReentrant
    {
        ...
        bytes32 positionHash = _slipstreamPositionHash(token0, token1, fee);
        ISlipstreamHelper(SLIPSTREAM_HELPER).unstakePosition(tokenId, positionHash);
        emit SlipstreamPositionUnstaked(positionHash, tokenId);
    }

    /// @notice Transfer Slipstream NFT to recipient (e.g., owner EOA)
    function transferSlipstreamPosition(uint256 tokenId, address recipient)
        external
        onlyOwner
        nonReentrant
    {
        ...
        ISlipstreamHelper(SLIPSTREAM_HELPER).transferPosition(tokenId, recipient);
        emit SlipstreamPositionTransferred(tokenId, recipient);
    }
    
    /// @notice Batch create LPs 
    function batchCreateVeloLP(LPParams[] calldata params) external onlyOwner whenNotPaused nonReentrant {
        if (emergencyMode) revert EmsModeInit();
        for (uint256 i = 0; i < params.length; i++) {
            LPParams calldata p = params[i];
            if (tokenMapping[p.tokenA] == address(0)) revert TokenNotSupported();
            if (_erc20Balance(p.tokenA) < p.amountA) revert InsufficientBalance();
            if (_erc20Balance(p.tokenB) < p.amountB) revert InsufficientBalance();
            p.tokenA.safeTransfer(VELO_HELPER, p.amountA);
            p.tokenB.safeTransfer(VELO_HELPER, p.amountB);
            IVelodromeHelper(VELO_HELPER).createLP(p.tokenA, p.tokenB, p.amountA, p.amountB, p.stable, p.stakeInGauge);
        }
    }

    /// @notice Harvest Velodrome rewards 
    function harvestVelodromeRewards(bytes32 pairHash) external onlyOwner whenNotPaused nonReentrant {
        if (emergencyMode) revert EmsModeInit();
        IVelodromeHelper(VELO_HELPER).harvestRewards(pairHash);
    }
    
    /// @notice Harvest Velodrome fees 
    function harvestVelodromeFees(address tokenA, address tokenB, bool stable) external onlyOwner whenNotPaused nonReentrant {
        if (emergencyMode) revert EmsModeInit();
    ...
    function getVeloGauge(address tokenA, address tokenB, bool stable) external view returns (address) {
    ...
    function getVeloLPBalance(address tokenA, address tokenB, bool stable) external view returns (uint256 balance) {
    ...
    function getVeloStakedLPBalance(address tokenA, address tokenB, bool stable) external view returns (uint256 staked) {
        bytes32 pairHash = _veloPairHash(tokenA, tokenB, stable);
    ...
    function getVeloTotalLPBalance(address tokenA, address tokenB, bool stable) external view returns (uint256 total) {
        bytes32 pairHash = _veloPairHash(tokenA, tokenB, stable);
    ...
    function unstakeVeloLP(address tokenA, address tokenB, bool stable, uint256 amount) external onlyOwner whenNotPaused nonReentrant {
        bytes32 pairHash = _veloPairHash(tokenA, tokenB, stable);
    ...
    function _slipstreamPositionHash(address tokenA, address tokenB, uint24 fee) internal pure returns (bytes32) {
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
    ...
    function setMaxDeposit(address token, uint128 maxAmount) external onlyOwner {
    ...
    modifier withdrawalLimitCheck(address token, uint256 amount) {
        ...
        if (maxDailyWithdrawals[token] > 0) {
            ...
            if (todaysWithdrawals + uint128(amount) > maxDailyWithdrawals[token]) {
                revert WithdrawalLimitExceeded();
            }
            dailyWithdrawals[token][today] = todaysWithdrawals + uint128(amount);
        }
        _;
    }
    ...
    function emsWithdrawFromTydro(address token, address to, uint256 amount) external onlyOwner {
        if (to == address(0)) revert InvalidAddress();
        if (tokenMapping[token] == address(0)) revert TokenNotSupported();
        bytes32 withdrawArgs = amount == 0 ?
            IL2Encoder(L2_ENCODER).encodeWithdrawParams(token, type(uint256).max) :
    ...
    event VaultInitialized(address tydroPool, address l2Encoder, address acrossSpokePool, address l1Recipient);
    ...
    event AutoDeposited(address indexed token, uint256 amount, address indexed caller, bool usedSmartAllocation);
    event AutoDepositSkipped(address indexed token, uint256 currentBalance, uint256 depositedAmount, address indexed caller);
    ...
    event BridgeError(address indexed token, bytes4 errorSelector, uint256 bridgeAmount, uint256 minAmountOut);

    event EmergencyWithdrawal(address indexed token, address indexed to, uint256 amount, address indexed caller);
    event EmergencyTydroWithdrawal(address indexed token, address indexed to, uint256 amount, address indexed caller);
    ...
    event OperationCooldownUpdated(address indexed token, uint32 oldCooldown, uint32 newCooldown);
    event MaxDailyWithdrawalUpdated(address indexed token, uint128 oldMax, uint128 newMax);
    event MaxDepositUpdated(address indexed token, uint128 oldMax, uint128 newMax);
    ...
    event VelodromeLPSentToOwner(address indexed pair, uint256 liquidity, address indexed owner);
    event SlipstreamGaugeSet(address indexed token0, address indexed token1, uint24 fee, address gauge);
    event SlipstreamPositionCreated(uint256 indexed tokenId, address indexed token0, address indexed token1, uint24 fee, bool staked);
    event SlipstreamLiquidityIncreased(uint256 indexed tokenId, uint256 liquidity, uint256 amount0, uint256 amount1);
    event SlipstreamLiquidityDecreased(uint256 indexed tokenId, uint128 liquidity, uint256 amount0, uint256 amount1);
    event SlipstreamFeesCollected(uint256 indexed tokenId, uint256 amount0, uint256 amount1);
    event SlipstreamRewardsHarvested(bytes32 indexed positionHash, uint256 rewards);
    event SlipstreamPositionStaked(bytes32 indexed positionHash, uint256 indexed tokenId);
    event SlipstreamPositionUnstaked(bytes32 indexed positionHash, uint256 indexed tokenId);
    event SlipstreamPositionTransferred(uint256 indexed tokenId, address indexed recipient);
}
