...
contract AdvYieldOptimizer is Ownable, ReentrancyGuard {
    ...
    struct AssetConfig {
        ...
        uint256 targetLTVBps;///Taget loan-to-value (e.g., 7500 = 75%)
        uint256 minHealthFactor;///inimum health factor (e.g., 150 = 1.5x)
    ...
    uint256 public constant HEALTH_FACTOR_PRECISION = 118;
    ...
    function openLeveragePosition(
       ...
       ///Supply borrowed amount (if same asset, creates lev loop)
    ...
    function compoundLeveragePosition(address collateralAsset) external onlyOwner nonReentrant {
       ...
       ///Repay some debt to maintain healh factor
    ...
    functon autoCompoundAll() external onlyOwner nonReentrant {
       //iterate through all positions and compound them
    ...
    functon _borrow(address asset, uint256 amount) internal {
       //Tydro uses compressed calldata for borrow too
       //need to be implemented based on Tydro's actual interface
       ///placeholder
    }

    functon _repay(address asset, uint256 amount) internal {
    ...
    functon _getDebtBalance(address asset) internal view returns (uint256) {
       ///Get variable debt token balance
        (, , , , , , , , , , address variableDebtToken, , , , ) = 
          TYDRO_POOL.getReserveData(asset);
       ///Would nee to query debt token balance
        return 0;
    }
    ...
    function _calculateHealthFactor(
        ...
        uint256 collateralAmount,
        uint256 debtAmount
    ) intrnal view returns (uint256) {
       //Simplified health factor calculation
       ///would use LTV and liquidation threshold from reserve data
