...
/// @dev Gas-optimized Solady implementation using Relay Protocol (native Ink bridge)
contract BundledYieldVaultV2_RELAY is Ownable, ReentrancyGuard {
    ...
    /*//////////////////////////////////////////////////////////////
                        DEPOSIT & YIELD OPERATIONS
    //////////////////////////////////////////////////////////////*/

    /// @notice Auto-deposit available bridged funds to Tydro (anyone can call - keeper-friendly)
    /// @param token L2 token address
    /// @dev Detects new token balance from bridge and automatically deposits to Tydro
    /// @dev Can be called by anyone (keeper/relayer) to auto-deposit bridged funds
    function depositAvailable(address token) external whenNotPaused nonReentrant {
        ...
        // Check if there's new balance to deposit
    ...
    function deposit(address token, uint256 amount) external onlyOwner whenNotPaused nonReentrant {
        ...
        // Transfer from sender (for owner deposit)
    ...
    function getYieldAvailable(address token) external view returns (uint256 yield) {
        ...
        // Calculate yield (current balance - deposited amount)
        ...
        nonReentrant
    {
        ...
        // Update yield (internal call - more efficient)
        _updateYield(token);
        ...
        // Bridge to L1 via Relay Protocol
        if (bridgeAmount > 0) {
            // Auto-refill gas if needed
            ...
            if (actualBridgeAmount > 0) {
                ...
                // If minBridgeAmount provided, verify fee doesn't exceed it
                ...
                // Check balance and approval before bridging
    ...
    function depositAndHarvest(
        ...
        nonReentrant
    {
        // First, auto-deposit any available bridged funds (call via this to access external function)
        this.depositAvailable(token);
        
        // Then harvest and bridge yield (call via this to access external function)
