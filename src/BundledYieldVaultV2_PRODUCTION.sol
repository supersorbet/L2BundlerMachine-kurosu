...
contract BundledYieldVaultV2_PRODUCTION is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        SlipstreamHelper slipHelper = new SlipstreamHelper(_slipstreamPositionNFT, _veloRouter);
    ...
    function zapIntoSlipstreamPosition(
        ...
        uint256 minAmount1,
        bool stakeInGauge
    ) external onlyOwner whenNotPaused nonReentrant rateLimitCheck(tokenIn) returns (uint256 tokenId) {
        if (emergencyMode) revert EmsModeInit();
        if (tokenMapping[tokenIn] == address(0)) revert TokenNotSupported();
        if (tokenMapping[tokenOut] == address(0)) revert TokenNotSupported();
        if (_erc20Balance(tokenIn) < amountIn) revert InsufficientBalance();
        ...
        emit SlipstreamPositionCreated(tokenId, tokenIn, tokenOut, fee, stakeInGauge);
    }
